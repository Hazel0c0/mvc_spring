...
public class LoginController {
    ...
    public String loginForm() {
        System.out.println("loginForm get요청");
        return "chap03/s-form";
    }

    @PostMapping("/s-login-check")
    public String loginCheck(String id, String pwd, Model model) {
        ...
        if (id.equals("111")){
            if (pwd.equals("9999")){
                model.addAttribute("logResult", "로그인성공");
            } else {
                model.addAttribute("logResult", "비밀번호가 틀렸습니다");
            }
        } else {
            model.addAttribute("logResult", "아이디가 존재하지 않습니다.");
        }
